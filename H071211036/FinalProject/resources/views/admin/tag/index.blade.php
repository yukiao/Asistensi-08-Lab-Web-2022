...
@section('content')
    ...
    <a href="/tag/create" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Add Tag</a>
                ...
                @foreach ($tag as $row)
                    ...
                    <td width="20%">
                        ...
                        <form action="/tag/{{$row->id}}" method="post">
                            @method('DELETE')
                            @csrf
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Anda yakin mengahapus data ?')"><i class="fas fa-trash"></i> Delete</button>
                        </form>
                        </div>
                    </td>
                    </tr>
